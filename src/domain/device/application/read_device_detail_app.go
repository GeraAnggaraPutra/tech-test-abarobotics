...
func readDeviceDetailApp(svc *service.Service, validate *validator.Validator) fiber.Handler {
	return func(c *fiber.Ctx) (err error) {
		...
		if err := validate.Validate(request); err != nil {
			logger.PrintError(err, "error validation read device request")
			return kernel.ResponseErrorValidate(c, err)
		}

		data, err := svc.ReadDeviceDetailService(c.UserContext(), request.GUID)
		if err != nil {
			return kernel.ResponseError(c, err, msgFailedGetDeviceDetail)
		}

		return kernel.ResponseData(c, kernel.ResponsePayload{
			Code:    http.StatusOK,
			Data:    payload.ToReadDeviceResponse(data),
			Message: msgSuccessGetDeviceDetail,
		})
	}
}
